...
'''
IGDB_Split_Merge_Seed_Aligner_local.py
Author: Xiaoli Shi
Date: 2025.01.14
...
class SplitBarcodeCleanReads:
    ...
    def run_crisprdatacraft_cmd(self, name_list, amplicon_db_dic):
        ...
        int_n_processes = 1
        int_n_processes = int(self.n_processes_for_pooled)

        print("Running Crisprdatacraft with %d processes" % self.n_processes_for_pooled)

        for idx in range(0, len(crisprdatacraft_cmds)):
            crisprdatacraft_cmd=crisprdatacraft_cmds[idx]
            logging.info('Running Crisprdatacraft on %s #%d/%d: %s' % (descriptor, idx, len(crisprdatacraft_cmds), crisprdatacraft_cmd))
            print(crisprdatacraft_cmd)
            return_value = sb.call(crisprdatacraft_cmd, shell=True)

            if return_value == 137:
                logging.warn('Crisprdatacraft was killed by your system (return value %d) on %s #%d: "%s"\nPlease reduce the number of processes (-p) and run again.'%(return_value, descriptor, idx, crisprdatacraft_cmd))
            elif return_value != 0:
                logging.warn('Crisprdatacraft command failed (return value %d) on %s #%d: "%s"'%(return_value, descriptor, idx, crisprdatacraft_cmd))
            else:
                logging.info('Finished Crisprdatacraft %s #%d' %(descriptor, idx))
