...
def plot_nucleotide_quilt(nuc_pct_df,mod_pct_df,fig_filename_root,save_also_png=False,sgRNA_intervals=None,min_text_pct=0.5,max_text_pct=0.95,quantification_window_idxs=None,sgRNA_names=None,sgRNA_mismatches=None,shade_unchanged=True,group_column='Batch'):
    ...
    for pos_ind in range(2, amp_len+1): #iterate over all nucleotide positions in the sequence (0=Batch, 1=Nucleotide, so start at 2)
        ...
        for i in range(nSamples): #iterate over all samples
            ...
            print("\n\nprint information1 ")
            print([(mod_pct_df[group_column] == sampleName) & (mod_pct_df['Modification'] == "Insertions_Left")])

            print("\n\nprint information1-1")
            print(mod_pct_df.loc[(mod_pct_df[group_column] == sampleName) & (mod_pct_df['Modification'] == "Insertions_Left")])

            print("\n\nprint information 1-2")
            print(pos_ind)

            print("\n\nprint information2 ")
            print(mod_pct_df.loc[(mod_pct_df[group_column] == sampleName) & (mod_pct_df['Modification'] == "Insertions_Left")].iloc[:, pos_ind])

            print("\n\nprint information3")
            tmp = mod_pct_df.loc[(mod_pct_df['Modification'] == "Insertions_Left")].values
            print("tmp0\n ",tmp[0][2])
            print("tmp1\n", tmp[0][2][0])


            print("\n\nprint information3-1")
            tmp = mod_pct_df.iloc[:, pos_ind]
            print(tmp)
            '''

            #ins_pct1 = float(mod_pct_df.loc[(mod_pct_df[group_column] == sampleName) & (mod_pct_df['Modification'] == "Insertions_Left")].iloc[:, pos_ind])
            tmp = mod_pct_df.loc[(mod_pct_df[group_column] == sampleName) & (mod_pct_df['Modification'] == "Insertions_Left")].values
            ins_pct = float(tmp[0,pos_ind])

            #print("\n\nprint information4")
            #print("ins_pct1 ", ins_pct1)
            #print("ins_pct ", ins_pct)
...
#    sampleReadCounts = list(nuc_pct_df.iloc[[((nSamples-1)-x)*nNucs for x in range(0,nSamples)],0]))
    ...
    # Ensure sums are converted to numeric
    n_sum = pd.to_numeric(n_sum, errors='coerce')
    ins_sum = pd.to_numeric(ins_sum, errors='coerce')
    del_sum = pd.to_numeric(del_sum, errors='coerce')
